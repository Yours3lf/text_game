...
class post_process
{
    ...
    void set_workgroup_size( vec2& gws, vec2& lws, vec2& dispatch_size, const uvec2& size )
    {
    ...
    void gen_mipmaps( GLuint texture, GLenum internal_format, uvec2 size, unsigned miplevels)
    {
    ...
    void gauss_blur( vec2 dir, uvec2 size, GLuint src_tex, int src_level, GLuint dst_tex, int dst_level )
    {
    ...
    void radial_blur( vec2 center, float density, float positive_weight, float negative_weight, float decay, uvec2 size, GLuint src_tex, int src_level, GLuint dst_tex, int dst_level )
    {
    ...
    void display_texture( uvec2 size, GLuint src_tex, int src_level )
    {
  ...
  public:
    ...
    void start_post_process()
    {
      ...
      //gauss_blur( vec2( 1, 0 ), screensize / 2, color_tex, 1, pingpong_tex, 1 );
      //gauss_blur( vec2( 0, 1 ), screensize / 2, pingpong_tex, 1, color_tex, 1 );
      
      radial_blur( vec2( 0.5 ), 0.9, 0.2, 0.4, 0.8, screensize / 2, color_tex, 1, pingpong_tex, 1 );

      display_texture( screensize / 2, pingpong_tex, 1 );
    }
};