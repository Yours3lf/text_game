  ...
  class mesh
  {
    public:
      ...
      static void update_animation( float time, scene& s, mat4* bones )
      {
        ...
        std::function< void ( mesh*, animation_node*, mat4 ) > traverse_node;
        traverse_node = [&]( mesh* m, animation_node* n, mat4 parent_transform )
        {
        ...
        {
          traverse_node( &s.meshes[c], s.meshes[c].root_node, mat4() );
        }
	    ...
	    static void load_into_meshes( const std::string& filename, scene& s, const bool& flip = false )
      {
        ...
        for( int c = 0; c < the_scene->mNumLights; ++c )
        {
          if( the_scene->mLights[c]->mType == aiLightSource_SPOT )
          {
            ...
            s.spot_lights.back().f.set_perspective( s.spot_lights.back().spot_cutoff, 1, 1, s.spot_lights.back().radius );
          }
        ...
        for( unsigned int c = 0; c < the_scene->mNumMeshes; ++c )
        {
          ...
          s.objects.back().transformation = mat4();
          ...
          for( unsigned int d = 0; d < the_scene->mMeshes[c]->mNumFaces; ++d )
          {
            ...
            s.meshes[cc].indices.push_back( uvec3( faces->mIndices[0], faces->mIndices[1], faces->mIndices[2] ) );
          }

          //write out vertices
		      s.meshes[cc].vertices.resize(the_scene->mMeshes[c]->mNumVertices);
		      memcpy(&s.meshes[cc].vertices[0].x, &the_scene->mMeshes[c]->mVertices[0], the_scene->mMeshes[c]->mNumVertices * sizeof(vec3));

          //write out normals
          if( the_scene->mMeshes[c]->mNormals )
          {
		        s.meshes[cc].normals.resize(the_scene->mMeshes[c]->mNumVertices);
		        memcpy(&s.meshes[cc].normals[0].x, &the_scene->mMeshes[c]->mNormals[0], the_scene->mMeshes[c]->mNumVertices * sizeof(vec3));
          }
          ...
          if( the_scene->mMeshes[c]->mTextureCoords[0] )
          {
            for( unsigned int d = 0; d < the_scene->mMeshes[c]->mNumVertices; ++d )
            {
              ...
              s.meshes[cc].tex_coords.push_back( vec2( tex_coord->x, tex_coord->y ) );
            }
          }
          ...
          if( s.meshes[cc].normals.size() > 0 && s.meshes[cc].tex_coords.size() > 0 )
          {
            ...
            std::for_each( s.meshes[cc].indices.begin(), s.meshes[cc].indices.end(),
                           [&]( mm::uvec3 & d )
            {
              int index_1 = d.x;
              int index_2 = d.y;
              ...
              mm::vec3 n = s.meshes[cc].normals[index_1];
              mm::vec3 v[3];
              v[0] = s.meshes[cc].vertices[index_1];
              v[1] = s.meshes[cc].vertices[index_2];
              v[2] = s.meshes[cc].vertices[index_3];
              mm::vec2 t[3];
              t[0] = s.meshes[cc].tex_coords[index_1];
              t[1] = s.meshes[cc].tex_coords[index_2];
              t[2] = s.meshes[cc].tex_coords[index_3];

              mm::vec3 tangent = mm::calc_tangent( v, t, n );

              s.meshes[cc].tangents[index_1] = tangent;
              s.meshes[cc].tangents[index_2] = tangent;
              s.meshes[cc].tangents[index_3] = tangent;
            } );
          }
        }
      ...
      void upload()
      {
        ...
        glBufferData( GL_ELEMENT_ARRAY_BUFFER, sizeof( uvec3 ) * indices.size(), &indices[0], GL_STATIC_DRAW );
